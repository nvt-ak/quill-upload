...
var BaseHandler = /*#__PURE__*/function () {
  ...
  return (0, _createClass2["default"])(BaseHandler, [{
          ...
          if (!ss[i].href || ss[i].href.startsWith(window.location.origin)) {
            try {
              ...
              for (var j = 0; j < rules.length; ++j) {
                if (rules[j].type === CSSRule.KEYFRAMES_RULE || rules[j].type === CSSRule.WEBKIT_KEYFRAMES_RULE) {
    ...
    value: function insertToEditor(url, index) {
      ...
      var defaultClass = "";
      switch (this.handler) {
        case _utils.Constants.blots.image:
          defaultClass = _utils.Constants.DEFAULT_IMAGE_CLASS;
          break;
        case _utils.Constants.blots.video:
          defaultClass = _utils.Constants.DEFAULT_VIDEO_CLASS;
          break;
        case _utils.Constants.blots.attachment:
          defaultClass = _utils.Constants.DEFAULT_ATTACHMENT_CLASS;
          break;
      }
      var existingNode = this.quill.root.querySelector("[data-placeholder-id=\"".concat(this.handlerId, "\"]"));
      var existingClasses = existingNode ? existingNode.getAttribute("class") : defaultClass;
      if (existingNode) {
        existingNode.remove();
      }
      this.quill.insertEmbed(index, this.handler, url);
      setTimeout(function () {
        var selector = _this.handler === _utils.Constants.blots.image ? "img[src=\"".concat(url, "\"]") : _this.handler === _utils.Constants.blots.video ? "iframe[src=\"".concat(url, "\"]") : "div[href=\"".concat(url, "\"]");
        var newNode = _this.quill.root.querySelector(selector);
        if (newNode) {
          if (existingClasses) {
          ...
          var parentDiv = newNode.closest(".ql-editor > *");
          if (parentDiv) {
            parentDiv.classList.add("".concat(defaultClass, "-wrapper"));
          }
        }
      }, 0);
    }
    ...
    value: function insertPlaceholder(id) {
      ...
      setTimeout(function () {
        var images = _this2.quill.root.querySelectorAll("img");
        ...
        try {
          for (_iterator.s(); !(_step = _iterator.n()).done;) {
            ...
            if (img.src.includes("placeholder.com") && !img.hasAttribute("data-placeholder-id")) {
              ...
              img.classList.add(_utils.Constants.DEFAULT_IMAGE_CLASS);
              var parentDiv = img.closest(".ql-editor > *");
              if (parentDiv) {
                parentDiv.classList.add("".concat(_utils.Constants.DEFAULT_IMAGE_CLASS, "-wrapper"));
              }
              break;
            }
          }
    ...
    value: function embedFile(file) {
      ...
      upload(file).then(function (url) {
        if (url) {
          _this3.insertFileToEditor(url, placeholderInfo.id);
        }
      })["catch"](function (error) {
        console.error("Upload error:", error);
        _this3.insertFileToEditor("https://via.placeholder.com/300?text=Upload+Failed", placeholderInfo.id);
      })["finally"](function () {
        _this3.placeholders["delete"](file);
      });
    }
  }, {
    ...
    value: function fileChanged() {
      ...
      files.forEach(function (file) {
        _this4.loadFile(null, file);
        _this4.embedFile(file);
      });
    ...
    value: function isVideo(extension) {
      return /(mp4|m4a|3gp|f4a|m4b|m4r|f4b|mov|flv|avi|ogg|webm)$/i.test(extension);
    }
  }, {
    key: "isAttachment",
